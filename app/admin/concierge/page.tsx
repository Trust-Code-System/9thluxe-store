...
export default async function ConciergeAdminPage() {
  ...
  const [conversations, usage, feedback] = await Promise.all([
    prisma.conciergeConversation.count(),
    prisma.conciergeUsageEvent.aggregate({ where: { createdAt: { gte: since } }, _count: true, _avg: { totalLatencyMs: true }, _sum: { inputTokens: true, outputTokens: true, estimatedCostMicros: true } }),
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <Card><CardHeader><CardTitle>Usage, last 24 hours</CardTitle></CardHeader><CardContent className="grid gap-2 text-sm sm:grid-cols-2"><Setting label="Input tokens" value={usage._sum.inputTokens ?? 0} /><Setting label="Output tokens" value={usage._sum.outputTokens ?? 0} /><Setting label="Estimated spend" value={`$${((usage._sum.estimatedCostMicros ?? 0) / 1_000_000).toFixed(2)}`} /><Setting label="Configured AI selector" value={configured.ai} /><Setting label="Feedback" value={feedback.map((x) => `${x.rating}: ${x._count}`).join(", ") || "None"} /></CardContent></Card>
      </div>
