...
export function CartDrawer({ open, cartCount, onClose }: CartDrawerProps) {
  ...
  const handleCoupon = async (e: React.FormEvent) => {
    ...
    if (data?.ok) {
      ...
      setCouponMessage(
        `Coupon applied – ₦${data.discountNGN.toLocaleString()} off`,
      );
    } else {
  ...
  return (
    <div className="fixed inset-0 z-[var(--z-drawer)] flex">
      ...
      />
      <aside className="relative ml-auto flex h-full w-full max-w-sm flex-col bg-background shadow-[var(--shadow-raised)]">
            ...
            items.map((item) => (
              <div key={item.productId} className="flex gap-3 px-4 py-4">
                <div className="relative h-16 w-16 shrink-0 overflow-hidden rounded-2xl bg-muted">
                  <Image
                    src={item.image || "/placeholder-flacon.svg"}
                    alt={item.name}
                    fill
        ...
        <div className="space-y-4 border-t border-border px-4 py-5">
          <form onSubmit={handleCoupon} className="space-y-2">
            ...
            {couponMessage && (
              <p className="text-xs text-foreground">{couponMessage}</p>
            )}
          </form>
