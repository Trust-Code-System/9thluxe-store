...
import { safetyAnswer, sanitizeConciergeText, validateSources } from "./response-policy"
import { searchFadeCatalogue, findSimilarCatalogueProducts } from "./tools/catalogue"
...
export async function orchestrateConciergeTurn(input: OrchestrateInput): Promise<ConciergeTurnResult> {
  ...
  if (intent.requiresCatalogue || (intent.requiresConversationContext && previousState.activeProductIds.length)) {
    ...
    const catalogue = intent.entities.perfumeNames.length > 1
      ...
      : intent.requiresConversationContext && previousState.activeProductIds.length
      ? await searchFadeCatalogue({ productIds: previousState.activeProductIds, inStockOnly: intent.requiresLiveStock, limit: 8 })
      : intent.primaryIntent === "SIMILAR_PERFUME"
        ? await findSimilarCatalogueProducts(previousState, 8)
        : await searchFadeCatalogue({ query: input.message, notes: intent.entities.notes, budgetMaxNGN: intent.entities.budgetMaxNGN ?? previousState.budgetMaxNGN, inStockOnly: intent.requiresLiveStock, sampleFirst: input.sampleFirst ?? previousState.sampleFirst, limit: 8 })
  ...
  answer = sanitizeConciergeText(answer)
  ...
  return {
    ...
    provider, model, usage: { inputTokens, outputTokens, searchCalls, toolCalls: toolCalls.slice(0, env.CONCIERGE_MAX_TOOL_CALLS) }, state: nextState,
  }
}
