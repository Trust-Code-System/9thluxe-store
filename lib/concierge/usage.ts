...
import { consumeRateLimit, clientIp, rateLimiterBackend } from "@/lib/middleware/limiter"
import type { ConciergeIdentity, ConciergeTurnResult } from "./types"
import { estimateConciergeCostMicros } from "./cost"
...
export async function assertConciergeEntitlement(req: { headers: { get(name: string): string | null } }, identity: ConciergeIdentity, requiresWebResearch: boolean) {
  ...
  if (env.NODE_ENV === "production" && rateLimiterBackend() !== "upstash") throw new AppError("SERVICE_UNAVAILABLE", { message: "The concierge is temporarily unavailable while durable usage protection is being configured." })
  const perMinute = await consumeRateLimit(`concierge-v2:user:${identity.userId}`, env.CONCIERGE_AUTH_PER_MINUTE, 60_000, false)
  if (!perMinute.ok) throw new AppError("RATE_LIMITED")
  const [daily, webDaily] = await Promise.all([
...
export async function recordConciergeUsage(input: {
  ...
  const estimatedCostMicros = result ? estimateConciergeCostMicros({
    provider: result.provider,
    model: result.model,
    inputTokens: result.usage.inputTokens,
    outputTokens: result.usage.outputTokens,
    searchCalls: result.usage.searchCalls,
  }) : 0
  await prisma.conciergeUsageEvent.create({ data: {
    ...
    firstTokenLatencyMs: result?.usage.firstTokenLatencyMs,
    totalLatencyMs: Date.now() - input.startedAt, completionStatus: input.completionStatus,
    errorCode: input.errorCode, citationsUsed: Boolean(result?.sources.length), estimatedCostMicros,
  } }).catch(() => undefined)
}
